...
func (r *MonthlyReport) Send(ctx context.Context) error {
	...
	data, filename, err := r.exporter.Execute(ctx, prevMonth)
	if err != nil {
		return fmt.Errorf("erro ao gerar relatório mensal: %w", err)
	}

	if data == nil {
		r.logger.Info("sem despesas no mês anterior, relatório não enviado",
			"month", prevMonth.Format("01/2006"))
		return nil
	}

	caption := fmt.Sprintf("📊 Relatório mensal de %s", prevMonth.Format("01/2006"))
